...
{
    public class A00044Controller : BaseController
    {
        private const string ReportName = "A00044";
        private readonly clsBaoCaoDuNoTinDungTheoNganhKinhTe_DBTK _clsBaoCaoDuNoTinDungTheoNganhKinhTeDbtk;

        public A00044Controller()
        {
        ...
        {
            var dinhKyBaoCao = _clsDinhKyBaoCao.DinhKyBaoCao(ReportName);
            var status = false;
            var reportCount = 0;
            var ngayDuLieu = (DateTime.Parse(denNgay)).ToString("yyyyMMdd");
            // tạo folder theo tháng
            var folderName = GetFolderName(denNgay, ReportName);
            var phamViBaoCao = _clsSoLieuToanHeThongVaDonVi.BaoCaoToanHeThongOrTruSoChinh(ReportName);
            foreach (var item in phamViBaoCao)
            {
                ...
                _createFileNameReport.GetFileNameReportMonthWpf(out ngayBaoCao, out maDviPsinhDlieu, out fileName,
                   maChiNhanh, _maDonViGui, ReportName, denNgay);
                var newFile = new FileInfo(fileName);
                var fileTemplate = new FileInfo(HttpContext.Server.MapPath($"~/Report/{ReportName}.xlsx"));
